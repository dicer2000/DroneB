...
def main():
    ...
    db = DroneB()
    threading.Thread(target=frame_grab, args=[db]).start() #Create a thread that runs the function frame_grab while main runs in the current thread
    
    while exiting.get() == False:
        db.process_keyboard()  # Process keystrokes
        lcurrent_frame = current_frame.get()
        if lcurrent_frame != None:
            image = db.process_frame(lcurrent_frame)
